...
export default function CompilerPage() {
  ...
  // ✅ Confirm reset - fixed to use STARTER_CODE
  const confirmReset = () => {
    ...
    <div className="h-screen flex flex-col bg-[#0f172a] text-white overflow-hidden">
      ...
      <div className="bg-[#1e293b] border-b border-[#334155] px-4 sm:px-6 py-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* ✅ Clear button now uses modal */}
          <button 
            onClick={handleClear} 
            className="flex items-center gap-1.5 bg-[#334155] hover:bg-[#475569] px-3 py-2 rounded-lg text-sm"
          >
            <Trash2 className="w-3.5 h-3.5" /> Clear
          </button>

          {/* ✅ Reset button uses modal */}
          <button 
            onClick={handleReset} 
            className="flex items-center gap-1.5 bg-[#334155] hover:bg-[#475569] px-3 py-2 rounded-lg text-sm"
          >
